...
do
    ...
    # NET read KB/s
    net_read_rate=`cat $nmon_file |grep "NET," |grep -v "Network I" | awk -F ',' '{sum+=$4} END{print sum/NR, "KB/s"}'`
    output_result_line="${output_result_line},${net_read_rate}"

    # NET write KB/s
    net_write_rate=`cat $nmon_file |grep "NET," |grep -v "Network I" | awk -F ',' '{sum+=$9} END{print sum/NR, "KB/s"}'`
